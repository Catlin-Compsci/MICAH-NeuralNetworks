...
public class DoodleDigitsWindowMP extends JFrame {
...
//        pen.fillRect(0,0,100,100);
        ...
        double[] inputData = new double[iIm.getWidth() * iIm.getHeight()];
        int i = 0;
        for (int y = 0; y < iIm.getHeight(); y++) {
            for (int x = 0; x < iIm.getWidth(); x++) {
                inputData[i] = (double) ((255 - new Color(iIm.getRGB(x, y)).getGreen()) * (double) 16 / (double) 256);
                i++;
            }
        }
        double[] prediction = new double[2]/*= nn.predict(inputData)*/;
        System.out.print(prediction + "\r");
        double[] largest = largestIndexValue(prediction);
        // IMPORTANT -- YOU SET THE THRESHOLD HERE, I should probably have this be a construction argument...
         number.setText(largest[1]>0.5d ? (int)largest[0] + "" : "?");
    }

    public static double[] largestIndexValue(double[] data) {
        int index = 0;
        double value = 0;
    ...
    public DoodleDigitsWindowMP(LinearNetwork digitClassfier) {
        super("Doodle Digits");
